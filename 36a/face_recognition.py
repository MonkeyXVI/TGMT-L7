...
class FaceRecognition:
    def __init__(self, path='./'):
        # Load models từ code gốc
        self.faceDetectionModel = os.path.join(path, '/thigiacmaytinh/36a/models/res10_300x300_ssd_iter_140000_fp16.caffemodel')
        self.faceDetectionProto = os.path.join(path, '/thigiacmaytinh/36a/models/deploy.proto.txt')
        self.faceDescriptor = os.path.join(path, '/thigiacmaytinh/36a/models/openface.nn4.small2.v1.t7')
    ...
    def pipeline_model(self, frame):
        # Sử dụng lại hàm pipeline_model từ code gốc
        image = frame.copy()
        h, w = frame.shape[:2]
        ...
        machinelearning_results = {
            ...
            'face_name_score': [],
            'count': [],
            'bbox': []  # Thêm bounding box để có thể vẽ khung nhận diện
        }
        count = 1
